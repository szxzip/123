## 模块: symbol.h — 符号表与常数表接口

## 简明解释

10 个公开函数的简要说明：

- **`sym_init`** — 初始化符号表系统，清零所有计数器（`sym_count`、`const_count`、`temp_count`、`label_count`），用 `memset` 清空符号表、常数表和语义栈数组。
- **`sym_enter_id`** — 标识符查填：先线性搜索是否已有同名标识符，有则返回索引（查），无则插入尾部并返回新索引（填）。一次扫描同时完成建表，是两趟扫描中第一趟的核心。
- **`sym_lookup_id`** — 纯查询标识符：线性搜索符号表，找到返回索引，未找到返回 `-1`。不插入新条目。
- **`sym_set_type`** — 更新符号的类型/宽度/偏移。由翻译文法 **a6** 语义动作调用，在确定变量声明类型后批量修正之前填入的占位信息。
- **`sym_enter_const`** — 常数查填：以 `1e-12` 容差进行浮点比较，已存在则返回索引，不存在则插入尾部。
- **`sym_lookup_const`** — 纯查询常数：同样的浮点容差比较逻辑，但不插入。
- **`sym_new_temp`** — 分配临时变量：递增 `temp_count`，生成名称 `"t1"`、`"t2"`…，登记到符号表（`KIND_TEMP`）。返回类型为 `char *`（静态缓冲区，每次调用覆盖）。
- **`sym_new_label`** — 分配跳转标号：仅递增 `label_count` 并返回编号，用于四元式 `OP_JMP`/`OP_JNZ` 等跳转目标。
- **`sym_dump`** — 将符号表格式化为可读表格字符串（索引/名字/种类/类型/偏移/宽度），使用 `snprintf` 渐进累积写入。
- **`const_dump`** — 将常数表格式化为可读表格字符串（索引/值），使用 `%g` 格式输出最简形式。
