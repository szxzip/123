...
| 7 | `if (s[0] == '-' && s[1] >= '0' && s[1] <= '9') return 1;` | 处理负数常数：如果第一个字符是 `-` 且第二个字符是数字 (`0`-`9`)，则认定为常数。例如 `"-5"`，这样生成汇编时写为 `$-5`。注意这里只检查了两个字符，所以负号紧跟数字即判断为常数（实际源码中的常数不会出现 `-x` 这种形式）。 |
| 9 | `return (s[0] >= '0' && s[0] <= '9');` | 处理正数常数：如果第一个字符是数字 `0`-`9`，返回 1（常数）；否则返回 0（变量名）。因为变量名由词法分析器产生，必定以字母开头（如 `x`、`count`、`t1`），而常数以数字或负号开头。返回值就是条件表达式的结果（0 或 1），简洁高效。 |
| 10 | `}` | 函数结束右大括号。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 12 | `// 将操作数字符串写入为汇编操作数` | 注释：说明下文给操作数加上适当的汇编语法前缀/后缀。 |
| 13 | `// * 常数 → $value ; 变量 → varname(%rip)` | 注释：常数输出时加 `$` 前缀（如 `$5`），变量输出时加 `(%rip)` 后缀（如 `x(%rip)`），后者表示 RIP 相对寻址——以当前指令指针 `%rip` 为基址的 32 位偏移访问数据段。 |
| 14 | `static void write_cmp_set(FILE *f, const char *set_instr,` | 函数定义：`static` 作用域限制；`void` 无返回值；参数分别为输出文件指针 `f`、条件置位指令字符串 `set_instr`（如 `"sete"`、`"setl"`）以及三个四元式字段 `a1`、`a2`、`r`。 |
| 14 | `                           const char *a1, const char *a2, const char *r) {` | 续行：`a1` 和 `a2` 是参与比较的两个操作数，`r` 是存放比较结果（0 或 1）的目标变量。 |
| 16 | `    // 将 a1 加载到 %eax` | 注释：步骤一——将第一个操作数加载到累加器 `%eax`。 |
| 17 | `    if (is_const_operand(a1)) {` | 判断 `a1` 是否为常数。 |
| 18 | `        fprintf(f, "    movl    $%s, %%eax\n", a1);` | 若 `a1` 是常数：生成 `movl $常数, %eax`。`movl` 是 32 位数据传送指令（"l" 后缀表示 long/32-bit）；`$%s` 输出 `$` + 常数值；`%%eax` 在 `printf` 格式串中双写 `%` 以输出一个字面 `%`。最终生成如 `movl $5, %eax`。 |
| 19 | `    } else {` | 若 `a1` 不是常数…… |
| 20 | `        fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 生成 `movl 变量名(%rip), %eax`。`(%rip)` 是 RIP 相对寻址，访问位于数据段的全局变量。最终生成如 `movl x(%rip), %eax`，含义为：将内存地址 `x` 处的 32 位值加载到 `%eax`。 |
| 21 | `    }` | 内层 if-else 结束。 |
| 22 | `    // 与 a2 比较` | 注释：步骤二——将 `%eax` 与 `a2` 进行比较。`cmpl S, D` 在 AT&T 语法中计算 `D - S`（即 `%eax - a2`），根据结果设置 EFLAGS 寄存器的标志位（ZF=零标志, SF=符号标志, OF=溢出标志, CF=进位标志）。比较之后不保留差值，只影响标志位。 |
| 23 | `    if (is_const_operand(a2)) {` | 判断 `a2` 是否为常数。 |
| 24 | `        fprintf(f, "    cmpl    $%s, %%eax\n", a2);` | 若 `a2` 是常数：生成 `cmpl $常数, %eax`。例如 `cmpl $10, %eax` 计算 `%eax - 10`，设置标志位。 |
| 25 | `    } else {` | 若 `a2` 不是常数…… |
| 26 | `        fprintf(f, "    cmpl    %s(%%rip), %%eax\n", a2);` | 生成 `cmpl 变量名(%rip), %eax`。例如 `cmpl y(%rip), %eax` 计算 `%eax - [y]`。 |
| 27 | `    }` | 内层 if-else 结束。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 33 | `void codegen_generate(Compiler *c, const char *outfile) {` | 函数定义：`void` 无返回值；`Compiler *c` 是编译器上下文指针，包含符号表、四元式列表等所有编译数据；`const char *outfile` 是输出汇编文件的路径名。 |
| 34 | `    FILE *f = fopen(outfile, "w");` | 以写入模式 (`"w"`) 打开输出文件。`fopen` 返回 `FILE*` 指针，后续用 `fprintf` 向该文件写入汇编代码。若文件已存在会被截断重写。 |
| 35 | `    if (!f) {` | 错误检查：若 `fopen` 返回 `NULL`（文件无法打开，如权限不足或路径不存在）…… |
| 36 | `        fprintf(stderr, "无法写入文件: %s\n", outfile);` | 向标准错误输出打印错误信息，告知用户哪个文件无法写入。 |
| 37 | `        return;` | 提前返回，不继续执行后续代码生成逻辑。 |
| 38 | `    }` | if 块结束。 |
| 37 | （空行） | 空白行。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 45 | `    // 收集需要在 .bss 中分配的变量 (kind != KIND_PROGRAM)` | 注释：遍历符号表，为非程序名（`KIND_PROGRAM`）的符号在 BSS 段中分配空间。`KIND_PROGRAM=0` 表示程序名符号，不需要分配空间。 |
| 46 | `    fprintf(f, "    .section .bss\n");` | 输出伪指令 `.section .bss`，声明后续内容放入 BSS 段（Block Started by Symbol）。BSS 段存放未初始化的全局变量和静态变量，在可执行文件中不占实际空间（只记录大小），程序加载时由操作系统清零。 |
| 47 | `    int i, max_offset = 0;` | 声明循环变量 `i` 和 `max_offset`（最大偏移量）。`max_offset` 会累加变量长度以找出整个数据段需要的最大字节数（虽然在当前实现中未实际使用 `max_offset`，但它记录了 .bss 段的理论总大小，方便调试和未来可能的栈帧优化）。 |
| 48 | `    for (i = 0; i < c->sym_count; i++) {` | 遍历符号表中的所有条目。`sym_count` 是符号表实际条目数，上限为 `MAX_SYMBOLS`。 |
| 49 | `        Symbol *s = &c->sym_table[i];` | 取第 `i` 个符号的指针 `s`，方便后续访问其字段 `name`、`kind`、`offset`、`len`。 |
| 50 | `        if (s->kind != KIND_PROGRAM) {` | 过滤掉程序名符号：`KIND_PROGRAM=0` 表示程序名（如 `program foo` 中的 `foo`），它不需要在数据段分配空间。其余符号（`KIND_VARIABLE=1` 变量、`KIND_TEMP=2` 临时变量）都需要内存空间。 |
...
| 53 | `            if (s->offset + s->len > max_offset)` | 比较当前变量的 `offset + len` 是否超过 `max_offset`。`offset` 是变量在模拟活动记录中的偏移（由语义分析阶段分配），`len` 是类型宽度。`offset + len` 表示该变量占用的末尾位置。 |
| 54 | `                max_offset = s->offset + s->len;` | 更新 `max_offset` 为当前最大末尾偏移。循环结束后 `max_offset` 就是所有变量中最远的 `offset + len`，即数据区所需的总字节数。这个值在当前实现中未直接用于生成汇编，但可以用来确认或设置 `.bss` 段大小。 |
| 55 | `        }` | 内层 if 结束。 |
| 56 | `    }` | for 循环结束。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 67 | `    // 遍历四元式, 生成汇编` | 注释：核心生成逻辑——逐个处理四元式列表。 |
| 68 | `    for (i = 0; i < c->quad_count; i++) {` | 循环遍历所有四元式。`quad_count` 是四元式个数，由解析器在语义分析阶段填充。 |
| 69 | `        Quadruple *q = &c->quads[i];` | 取第 `i` 个四元式的指针 `q`，避免反复用数组下标访问。 |
| 70 | `        const char *a1 = q->arg1;` | 将四元式的三个操作数字段提取为局部变量：`a1`（第一操作数）。 |
| 71 | `        const char *a2 = q->arg2;` | `a2`（第二操作数）。某些操作码（如 `OP_JMP`、`OP_LABEL`）不使用这两个字段，其值为 `"_"`。 |
| 72 | `        const char *r  = q->result;` | `r`（结果/目标）。在不同操作码中含义不同：可能是存放结果的目标变量，也可能是跳转的目标标号（`OP_JMP`、`OP_JNZ` 中）。 |
| 71 | （空行） | 空白行。 |
| 74 | `        fprintf(f, "    # %-6s %-6s %-6s %s\n",` | 在每条汇编指令之前输出一行注释。`#` 是 GAS 汇编的单行注释符；`%-6s` 是左对齐宽度 6 的字符串格式，使输出列对齐。 |
| 74 | `                op_names[q->op], a1, a2, r);` | 注释内容为：操作码名称（如 `:=`、`+`、`jmp` 等）和三个操作数。`op_names[]` 是定义在 `quadruple.c` 中的全局字符串数组，将操作码枚举值映射为人类可读的名称（如 `op_names[OP_ADD]` = `"+"`）。这让生成的 `.s` 文件能直观看到每条汇编指令对应的四元式。 |
| 74 | （空行） | 空白行。 |
| 76 | `        switch (q->op) {` | 根据四元式的操作码 (`op`) 分支到不同的汇编生成逻辑。这是经典的基于操作码的代码生成调度。 |
| 76 | （空行） | 空白行。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 78 | `        case OP_PROGRAM:` | 匹配程序入口四元式 `(program, id, _, _)`。这是四元式列表的第一条，来自解析 `program id` 的文法规则。参数 `id` 在本实现中未使用，因为入口函数名固定为 `main`。 |
| 79 | `            // 无需额外代码, main 入口已在上面` | 注释：`main` 函数的序言（pushq rbp / movq rsp rbp）已经在循环之前输出，此 case 无需生成任何额外代码。 |
| 80 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 82 | `        case OP_ASSIGN:` | 匹配赋值操作码（枚举值 2）。 |
| 83 | `            // (:=, src, _, dst)` | 注释：说明四元式格式——第一参数是源操作数，第二参数固定为 `_`（未使用），第三参数是目标变量。 |
| 84 | `            // * movl src, %eax ; movl %eax, dst` | 注释：说明生成的汇编逻辑——两步数据传送。为什么要经过 `%eax` 中转？x86-64 的 `movl` 不允许两个操作数都是内存地址（不允许 memory-to-memory 传送），所以必须先将源加载到寄存器，再从寄存器写入目标。 |
| 85 | `            if (is_const_operand(a1)) {` | 判断源操作数 `a1` 是否为常数。 |
| 86 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 若 `a1` 是常数（如 `"5"`），生成 `movl $5, %eax`，将立即数 5 加载到 `%eax`。`$` 前缀在 AT&T 语法中表示立即数。 |
| 87 | `            } else {` | 若 `a1` 不是常数…… |
| 88 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 生成 `movl 变量名(%rip), %eax`，从内存加载变量值到 `%eax`。例如 `x := 5` → `movl $5, %eax`；`x := y` → `movl y(%rip), %eax`。 |
| 89 | `            }` | if-else 结束。 |
| 90 | `            fprintf(f, "    movl    %%eax, %s(%%rip)\n", r);` | 将 `%eax` 的值写入目标变量 `r`。RIP 相对寻址访问数据段中的变量。例如 `movl %eax, x(%rip)`。 |
| 91 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 93 | `        case OP_ADD:` | 匹配加法操作码（枚举值 3）。 |
| 94 | `            if (is_const_operand(a1)) {` | 判断第一个操作数是否为常数。 |
| 95 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 若 `a1` 是常数，用 `movl $值, %eax` 立即数加载。 |
| 96 | `            } else {` | 否则…… |
| 97 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 从变量内存加载到 `%eax`。 |
| 98 | `            }` | 加载部分 if-else 结束。 |
| 99 | `            if (is_const_operand(a2)) {` | 判断第二个操作数是否为常数。 |
| 100 | `                fprintf(f, "    addl    $%s, %%eax\n", a2);` | 若 `a2` 为常数，生成 `addl $值, %eax`。`addl` 是 32 位加法指令，将源操作数加到目标操作数上：`%eax = %eax + a2`。例如 `addl $3, %eax`。 |
| 101 | `            } else {` | 否则…… |
| 102 | `                fprintf(f, "    addl    %s(%%rip), %%eax\n", a2);` | 用变量值加到 `%eax`：`addl y(%rip), %eax`。 |
| 103 | `            }` | 加法部分 if-else 结束。 |
| 104 | `            fprintf(f, "    movl    %%eax, %s(%%rip)\n", r);` | 将 `%eax`（此时已为 `a1 + a2` 的和）写入目标变量 `r`。 |
| 105 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 107 | `        case OP_SUB:` | 匹配减法操作码（枚举值 4）。 |
| 108 | `            if (is_const_operand(a1)) {` | 判断第一操作数是否为常数。 |
| 109 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 常数加载到 `%eax`。 |
| 110 | `            } else {` | 否则…… |
| 111 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 从变量加载到 `%eax`。 |
| 112 | `            }` | if-else 结束。 |
| 113 | `            if (is_const_operand(a2)) {` | 判断减数 `a2` 是否为常数。 |
| 114 | `                fprintf(f, "    subl    $%s, %%eax\n", a2);` | `subl $值, %eax`：从 `%eax` 减去立即数。AT&T 语法中 `subl S, D` 计算 `D = D - S`。 |
| 115 | `            } else {` | 否则…… |
| 116 | `                fprintf(f, "    subl    %s(%%rip), %%eax\n", a2);` | `subl y(%rip), %eax`：从 `%eax` 减去变量 `y` 的值。 |
| 117 | `            }` | if-else 结束。 |
| 118 | `            fprintf(f, "    movl    %%eax, %s(%%rip)\n", r);` | 将结果写回目标变量。 |
| 119 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 135 | `        case OP_DIV:` | 匹配除法操作码（枚举值 6）。 |
| 136 | `            if (is_const_operand(a1)) {` | 判断被除数 `a1` 是否为常数。 |
| 137 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 常数加载到 `%eax`。 |
| 138 | `            } else {` | 否则…… |
| 139 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 从变量加载被除数到 `%eax`。 |
| 140 | `            }` | if-else 结束。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 152 | `        case OP_AND_OP:` | 匹配逻辑与操作码（枚举值 15）。命名为 `OP_AND_OP` 而非 `OP_AND` 是为了避免与 C 标准库中的 `AND` 宏/保留字冲突。 |
| 153 | `            if (is_const_operand(a1)) {` | 判断第一操作数是否为常数。 |
| 154 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 常数加载到 `%eax`。 |
| 155 | `            } else {` | 否则…… |
| 156 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 从变量加载到 `%eax`。 |
| 157 | `            }` | if-else 结束。 |
| 158 | `            if (is_const_operand(a2)) {` | 判断第二操作数是否为常数。 |
| 159 | `                fprintf(f, "    andl    $%s, %%eax\n", a2);` | `andl $值, %eax`：将 `%eax` 与立即数按位与。`andl` 是 32 位按位与指令，`D = D & S`。 |
| 160 | `            } else {` | 否则…… |
| 161 | `                fprintf(f, "    andl    %s(%%rip), %%eax\n", a2);` | `andl 变量(%rip), %eax`：`%eax = %eax & 变量值`。 |
| 162 | `            }` | if-else 结束。 |
| 163 | `            fprintf(f, "    movl    %%eax, %s(%%rip)\n", r);` | 将按位与结果写入目标变量。 |
| 164 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 166 | `        case OP_OR_OP:` | 匹配逻辑或操作码（枚举值 16）。 |
| 167 | `            if (is_const_operand(a1)) {` | 判断第一操作数是否为常数。 |
| 168 | `                fprintf(f, "    movl    $%s, %%eax\n", a1);` | 常数加载到 `%eax`。 |
| 169 | `            } else {` | 否则…… |
| 170 | `                fprintf(f, "    movl    %s(%%rip), %%eax\n", a1);` | 从变量加载到 `%eax`。 |
| 171 | `            }` | if-else 结束。 |
| 172 | `            if (is_const_operand(a2)) {` | 判断第二操作数是否为常数。 |
| 173 | `                fprintf(f, "    orl     $%s, %%eax\n", a2);` | `orl $值, %eax`：将 `%eax` 与立即数按位或。 |
| 174 | `            } else {` | 否则…… |
| 175 | `                fprintf(f, "    orl     %s(%%rip), %%eax\n", a2);` | `orl 变量(%rip), %eax`：`%eax = %eax | 变量值`。 |
| 176 | `            }` | if-else 结束。 |
| 177 | `            fprintf(f, "    movl    %%eax, %s(%%rip)\n", r);` | 将结果写入目标变量。 |
| 178 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 192 | `        case OP_JE:` | `OP_JE`（枚举值 9）：等于比较 `a1 == a2`。注意此处的 `OP_JE`（比较并产生布尔结果）与 `OP_JMP` 不同——`OP_JE` 产生一个可赋值的布尔值，`OP_JMP` 是直接跳转。 |
| 193 | `            write_cmp_set(f, "sete", a1, a2, r);` | 委托给 `write_cmp_set`，传入 `"sete"`（set if equal）。若 `a1 == a2` 则 r=1，否则 r=0。 |
| 194 | `            break;` | 跳出 switch。 |
| 195 | `        case OP_JNE:` | `OP_JNE`（枚举值 10）：不等于比较 `a1 != a2`。 |
| 196 | `            write_cmp_set(f, "setne", a1, a2, r);` | `setne`（set if not equal）：若 `a1 != a2` 则 r=1。 |
| 197 | `            break;` | 跳出 switch。 |
| 198 | `        case OP_JL:` | `OP_JL`（枚举值 11）：小于比较 `a1 < a2`（有符号）。 |
| 199 | `            write_cmp_set(f, "setl", a1, a2, r);` | `setl`（set if less）：若 SF≠OF（有符号小于），则 r=1。标志位逻辑：`cmp a2, a1` 计算 `a1 - a2`，若结果为负且没有溢出（或为正但发生了溢出），则表示 `a1 < a2`。 |
| 200 | `            break;` | 跳出 switch。 |
| 201 | `        case OP_JG:` | `OP_JG`（枚举值 12）：大于比较 `a1 > a2`（有符号）。 |
| 202 | `            write_cmp_set(f, "setg", a1, a2, r);` | `setg`（set if greater）：若 ZF=0 且 SF=OF（有符号大于），则 r=1。条件为 `!ZF && SF==OF`。 |
| 203 | `            break;` | 跳出 switch。 |
| 204 | `        case OP_JLE:` | `OP_JLE`（枚举值 13）：小于等于比较 `a1 <= a2`（有符号）。 |
| 205 | `            write_cmp_set(f, "setle", a1, a2, r);` | `setle`（set if less or equal）：若 ZF=1 或 SF≠OF，则 r=1。 |
| 206 | `            break;` | 跳出 switch。 |
| 207 | `        case OP_JGE:` | `OP_JGE`（枚举值 14）：大于等于比较 `a1 >= a2`（有符号）。 |
| 208 | `            write_cmp_set(f, "setge", a1, a2, r);` | `setge`（set if greater or equal）：若 SF=OF，则 r=1。 |
| 209 | `            break;` | 跳出 switch。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 211 | `        case OP_JMP:` | `OP_JMP`（枚举值 7）：无条件跳转。 |
| 212 | `            // (jmp, _, _, label)` | 注释：四元式格式——`a1` 和 `a2` 均为 `_`（未使用），`r`（结果字段）存放目标标号名称（如 `L1`、`L_end`）。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 216 | `        case OP_JNZ:` | `OP_JNZ`（枚举值 8）：若条件为真（非零）则跳转。用于实现 IF 语句的条件分支和 WHILE 循环的循环条件检查。 |
| 217 | `            // (jnz, cond, _, label)` | 注释：四元式格式——`a1` 是条件变量（布尔值 0/1），`a2` 为 `_`，`r` 是条件为真时的跳转目标标号。 |
| 218 | `            // * 如果 cond 是标签引用 (L*), 直接作为跳转目标` | 注释：说明一种特殊用法——如果条件操作数 `a1` 本身就是标号（以 `L` 开头），则直接作为跳转目标。但实际上这里走的是标准 `cmpl + jne` 路径，不会出现 "直接作为跳转目标" 的情况（这个注释可能是遗留的或描述其他模块的行为）。 |
| 219 | `            if (is_const_operand(a1)) {` | 判断条件变量 `a1` 是否为常数。如果条件本身是一个硬编码的常量（如 `jnz 1 _ L1`），则直接比较常数值 0。 |
| 220 | `                fprintf(f, "    cmpl    $0, $%s\n", a1);` | 与常数 0 比较：`cmpl $0, $常量`。注意这行有微妙之处——`cmpl` 的两个操作数都可以是立即数，但实际上这种用法不常见且可能不正确（`cmpl` 不能比较两个立即数）。在实际代码中，`a1` 几乎不会是常量条件。 |
| 221 | `            } else {` | 若条件变量是内存变量…… |
| 222 | `                fprintf(f, "    cmpl    $0, %s(%%rip)\n", a1);` | `cmpl $0, 变量(%rip)`：将变量值与 0 比较。例如 `cmpl $0, t1(%rip)`，检查 `t1` 是否为零。AT&T 语法 `cmpl S, D` 计算 `D - S`（即 `变量 - 0`），设置标志位。 |
| 223 | `            }` | if-else 结束。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 243 | `        case OP_LABEL:` | `OP_LABEL`（枚举值 19）：标签/标号操作码，在汇编中生成一个跳转目标标号。 |
| 244 | `            // (label, Lx, _, _) → 汇编标签` | 注释：四元式格式——`a1` 存放标号名（如 `L1`、`L_loop`、`L_end`），`a2` 和 `r` 未使用。此四元式不生成指令，只在汇编中定义一个地址标号。 |
...
| 行号 | 代码 | 讲解 |
|------|------|------|
| 248 | `        default:` | 默认分支，处理未知操作码（理论上不会发生，但作为防御性编程）。 |
| 249 | `            fprintf(f, "    # 未支持的操作码 %d\n", q->op);` | 输出一条汇编注释，标注未支持的操作码编号。这有助于调试——如果生成的 `.s` 文件中出现此注释，说明代码生成遗漏了某个操作码。 |
| 250 | `            break;` | 跳出 switch。 |
| 251 | `        }` | switch 语句结束。 |
