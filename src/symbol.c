...
int sym_enter_id(Compiler *c, const char *name, int type, int kind,
                 int offset) {
  int i;
  // 先查找
...
// 分配新标号, 返回编号
int sym_new_label(Compiler *c) { return ++c->label_count; }
...
void sym_dump(Compiler *c, char *buf, int bufsize) {
  ...
  for (i = 0; i < c->sym_count; i++) {
    ...
    const char *kind_str = (s->kind == KIND_PROGRAM)    ? "程序"
                           : (s->kind == KIND_VARIABLE) ? "变量"
                                                        : "临时";
    pos +=
        snprintf(buf + pos, bufsize - pos, "%-6d %-16s %-8s %-8s %-6d %d\n", i,
                 s->name, kind_str, type_names[s->type], s->offset, s->len);
  }
}
...
void const_dump(Compiler *c, char *buf, int bufsize) {
  ...
  for (i = 0; i < c->const_count; i++) {
    pos +=
        snprintf(buf + pos, bufsize - pos, "%-6d %g\n", i, c->const_table[i]);
  }
}
