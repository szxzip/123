...
const char *keys_table[] = {
    ...
    ";",       ":=",   "*",       "/",     "+",    "-",     ".",   "(",   ")",
    "if",      "then", "else",    "while", "do",   "and",   "or",  "not", "=",
    "<",       ">",    "<=",      ">=",    "<>",   "write", NULL};

// Pascal 常数自动机: 状态转换矩阵
// * 行=状态-1, 列: digit(0)/dot(1)/Ee(2)/+-(3)/other(4)
// * 状态 1~8: 1=初态, 2=整数, 3=刚读., 4=小数, 5=刚读E, 6=刚读符号, 7=指数,
// 8=接受
const int const_aut[8][5] = {{2, 0, 0, 0, 0}, {2, 3, 5, 8, 8}, {4, 0, 0, 0, 0},
                             {4, 0, 5, 8, 8}, {7, 0, 0, 6, 0}, {7, 0, 0, 0, 0},
                             {7, 0, 0, 8, 8}, {0, 0, 0, 0, 0}};
...
void scanner_next_token(Compiler *c) {
  ...
  if (c->token.code == 0) {
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "未识别的符号: '%c' (0x%02X)",
             str_token[0], (unsigned char)str_token[0]);
  }
  c->token.value = -1;
}
...
void scanner_dump_tokens(Compiler *c, char *buf, int bufsize) {
  ...
  for (i = 0; i < c->token_count; i++) {
    ...
    if (t->code == TOK_EOF)
      break;
    if (t->code == TOK_ID)
      pos += snprintf(buf + pos, bufsize - pos, "(k,1)(i,%d) ", t->value);
    else if (t->code == TOK_CONST)
      pos += snprintf(buf + pos, bufsize - pos, "(c,%d) ", t->value);
    else
      pos += snprintf(buf + pos, bufsize - pos, "(p,%d) ", t->code);
    if ((i + 1) % 8 == 0)
      pos += snprintf(buf + pos, bufsize - pos, "\n");
  }
  pos += snprintf(buf + pos, bufsize - pos, "\n");
