...
void sem_a1(Compiler *c) { c->cur_offset = 0; }
...
void sem_emit_comparison(Compiler *c, int rel_op, SemValue *left,
                         SemValue *right) {
  int quad_op;
  switch (rel_op) {
  case TOK_EQ:
...
void sem_emit_end(Compiler *c) { quad_emit(c, OP_END, "_", "_", "_"); }

// ========== IF 语句语义 ==========

void sem_if_begin(Compiler *c, SemValue *cond, int *out_true, int *out_false,
                  int *out_end) {
...
void sem_while_check(Compiler *c, SemValue *cond, int label_body,
                     int label_exit) {
  char buf[MAX_STR];
