...
static void PROGRAM(Compiler *c) {
  if (c->token.code == TOK_PROGRAM) {
    ...
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: 期望 'program'");
  }
}
...
static void ID_SEQUENCE(Compiler *c) {
  if (c->token.code == TOK_ID) {
    ...
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: 期望标识符");
  }
}
...
static void COM_SENTENCE(Compiler *c) {
  if (c->token.code == TOK_BEGIN) {
    ...
    if (c->token.code == TOK_END)
      ...
      c->error_flag = 1;
      snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: 期望 'end'");
    }
  } else {
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: 期望 'begin'");
  }
}
...
static void STATEMENT(Compiler *c) {
  ...
  case TOK_WRITE:
    ...
    if (c->token.code == TOK_ID) {
      int idx = c->token.value;
      const char *id_name =
          (idx >= 0 && idx < c->sym_count) ? c->sym_table[idx].name : "?";
...
static void IF_STATEMENT(Compiler *c) {
  ...
  if (c->token.code == TOK_THEN) {
    ...
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: if 后期望 'then'");
  }
}
...
static void WHILE_STATEMENT(Compiler *c) {
  ...
  if (c->token.code == TOK_DO) {
    ...
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: while 后期望 'do'");
  }
}
...
int parser_parse(Compiler *c) {
  ...
  if (c->token.code != TOK_EOF) {
    c->error_flag = 1;
    snprintf(c->error_msg, sizeof(c->error_msg), "语法错误: 多余输入 Token %d",
             c->token.code);
    return 0;
  }
