...
static GtkWidget *src_view, *token_view, *sym_view, *quad_view, *asm_view,
    *run_view;
...
static void on_open_clicked(GtkWidget *widget, gpointer data) {
  ...
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "打开源文件", GTK_WINDOW(parent_window), GTK_FILE_CHOOSER_ACTION_OPEN,
...
static void on_gcc_clicked(GtkWidget *widget, gpointer data) {
  ...
  // 运行 gcc 编译
  pos += snprintf(result + pos, sizeof(result) - pos, "===== GCC 编译 =====\n");
  FILE *gcc = popen("gcc -no-pie /tmp/opencode/compiler_output.s -o "
                    "/tmp/opencode/compiler_output 2>&1",
                    "r");
  if (gcc) {
    char line[512];
    ...
    if (status == 0) {
      pos += snprintf(result + pos, sizeof(result) - pos, "GCC 编译成功\n\n");
      // 运行可执行文件
...
static void activate(GtkApplication *app, gpointer user_data) {
  ...
  g_signal_connect(btn_compile, "clicked", G_CALLBACK(on_compile_clicked),
                   NULL);
...
void run_gtk(int argc, char **argv) {
  GtkApplication *app =
      gtk_application_new("edu.compiler.frontend", G_APPLICATION_DEFAULT_FLAGS);
