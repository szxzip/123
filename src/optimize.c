...
// 解析整常数值
static int const_val(const char *s) { return atoi(s); }
...
static int fold_constants(Compiler *c) {
  ...
  for (int i = 0; i < c->quad_count; i++) {
    ...
    if (q->op == OP_ADD || q->op == OP_SUB || q->op == OP_MUL ||
        q->op == OP_DIV) {
...
static int propagate_constants(Compiler *c) {
  ...
  for (i = 0; i < c->quad_count; i++) {
    Quadruple *q = &c->quads[i];
    // 找到常数赋值: (:= C _ var)
    if (q->op == OP_ASSIGN && is_int_const(q->arg1) && q->arg2[0] == '_' &&
        !is_int_const(q->result)) {
      ...
      for (j = i + 1; j < c->quad_count; j++) {
        ...
        if (u->op == OP_ASSIGN && strcmp(u->result, var) == 0)
          break;
        // 如果变量被用作操作数, 替换为常数
        if (u->op == OP_ADD || u->op == OP_SUB || u->op == OP_MUL ||
            u->op == OP_DIV) {
